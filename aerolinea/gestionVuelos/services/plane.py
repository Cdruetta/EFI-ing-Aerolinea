...
class PlaneService:
    ...
    @staticmethod
    def create(model: str, capacity: int, available_seats: int) -> Plane:
        plane = Plane(model=model, capacity=capacity, available_seats=available_seats)
        plane.full_clean()
        plane.save()
        return plane

    @staticmethod
    def update(plane_id: int, model: str, capacity: int, available_seats: int) -> bool:
        try:
