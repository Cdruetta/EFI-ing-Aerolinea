...
from gestionVuelos.models import (
    Passenger,
    Flight,
    Plane,
    Reservation,
...
class PassengerUpdateView(LoginRequiredMixin, StaffRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "gestionVuelos:passenger_detail", kwargs={"pk": self.object.pk}
        )
...
class FlightUpdateView(StaffRequiredMixin, LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "gestionVuelos:flight_detail", kwargs={"pk": self.object.pk}
        )
...
class ReservationCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "gestionVuelos:reservation_detail", kwargs={"pk": self.reservation.pk}
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        available_seats = Seat.objects.filter(
            plane=flight.plane, status="available"
        ).order_by("row", "column")
        ...
        context["seat_types"] = (
            dict(Seat.SEAT_STATUS_CHOICES)
            if hasattr(Seat, "SEAT_STATUS_CHOICES")
            else {}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            seat = Seat.objects.get(pk=seat_id, plane=flight.plane, status="available")
...
class MyReservationsListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Reservation.objects.filter(
            passenger__user=self.request.user, status="reserved"
        )
