# -*- coding: utf-8 -*-
"""
API Views para el sistema de gestion de vuelos (app separada `api`)
"""
...
class FlightViewSet(viewsets.ModelViewSet):
    ...
    def passengers(self, request, pk=None):
        ...
        reservations = flight.reservations.filter(status='reserved').select_related('passenger', 'seat')
...
class PassengerViewSet(viewsets.ModelViewSet):
    ...
    def reservations(self, request, pk=None):
        ...
        reservations = Reservation.objects.filter(passenger=passenger).select_related('flight__plane', 'seat')
    ...
    def active_reservations(self, request, pk=None):
        passenger = self.get_object()
        reservations = Reservation.objects.filter(passenger=passenger, status='reserved').select_related('flight__plane', 'seat')
...
class ReservationViewSet(viewsets.ModelViewSet):
    ...
    def check_seat(self, request):
        ...
        if seat.plane != flight.plane:
            return Response({'error': 'El asiento no pertenece al avión de este vuelo'}, status=status.HTTP_400_BAD_REQUEST)

        occupied = flight.reservations.filter(seat=seat, status='reserved').exists()

        return Response({'seat_id': seat_id, 'available': not occupied, 'seat_number': seat.number, 'seat_type': seat.seat_type})
    ...
    def confirm(self, request, pk=None):
        ...
        if reservation.status != 'reserved':
            return Response({'error': 'Solo se pueden confirmar reservas en estado "reserved"'}, status=status.HTTP_400_BAD_REQUEST)

        reservation.status = 'confirmed'
        reservation.save()

        serializer = self.get_serializer(reservation)
        return Response({'message': 'Reserva confirmada exitosamente', 'reservation': serializer.data})
...
class PlaneViewSet(viewsets.ModelViewSet):
    ...
    def seats(self, request, pk=None):
        ...
        seats_data = []
        for seat in seats:
            seats_data.append({'id': seat.id, 'number': seat.number, 'row': seat.row, 'column': seat.column, 'seat_type': seat.seat_type, 'status': seat.status})
        return Response({'plane': {'id': plane.id, 'model': plane.model, 'manufacturer': plane.manufacturer, 'capacity': plane.capacity}, 'total_seats': len(seats_data), 'seats': seats_data})
...
class TicketViewSet(viewsets.ModelViewSet):
    ...
    def by_barcode(self, request):
        ...
        if not barcode:
            return Response({'error': 'El parámetro barcode es requerido'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            ticket = Ticket.objects.select_related('reservation__passenger', 'reservation__flight__plane', 'reservation__seat').get(barcode=barcode)
            ticket_data = {
                'id': ticket.id,
            ...
            return Response({'error': 'No se encontró un boleto con el código proporcionado'}, status=status.HTTP_404_NOT_FOUND)

    @action(detail=True, methods=['post'], permission_classes=[IsAdminUser])
    def generate(self, request, pk=None):
        reservation = self.get_object().reservation
        if reservation.status != 'reserved':
            return Response({'error': 'Solo se pueden generar boletos de reservas confirmadas'}, status=status.HTTP_400_BAD_REQUEST)
        if hasattr(reservation, 'ticket'):
            return Response({'error': 'Ya existe un boleto para esta reserva'}, status=status.HTTP_400_BAD_REQUEST)
        ticket = Ticket.objects.create(reservation=reservation)
        serializer = self.get_serializer(ticket)
        return Response({'message': 'Boleto generado exitosamente', 'ticket': serializer.data}, status=status.HTTP_201_CREATED)
